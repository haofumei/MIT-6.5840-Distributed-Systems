...
func (kv *ShardKV) doit(op *ClientOp) doitResult {
	result := doitResult{ClientId: op.ClientId, SN: op.SN}

	kv.mu.Lock()

	// check if the replica group is responsible for this op
...
func (kv *ShardKV) ingestCommand(index int, command interface{}) {
	...
	if op, ok := command.(ServerOp); ok {
		kv.Shards[op.Sid].Status = ShardMigratable
		if op.Type == "Migration" {
			go kv.sendShardMigration(op.Servers, op.Sid, op.Num)
		}
		return
	}
	...
	if dEntry, ok := kv.DupTable[op.ClientId]; ok && dEntry.SN >= op.SN { 
		...
		switch op.Type {
		case "Get":
			value, ok := kv.Shards[op.Shard].Data[op.Key]
			if ok {
				result.Value = value
				result.Err = OK			
			} else {
		...
		case "Put":
			kv.Shards[op.Shard].Data[op.Key] = op.Value
			result.Err = OK
		case "Append":
			kv.Shards[op.Shard].Data[op.Key] += op.Value
			result.Err = OK
...
func (kv *ShardKV) pollTicker() {
	for !kv.killed() {
		...
		if lastConfig.Num == newConfig.Num { // same config
			kv.mu.Unlock()
			time.Sleep(PollInterval * time.Millisecond)
			continue
		}

		DPrintf("(%d:%d) last C: %v and new C: %v",kv.gid, kv.me, lastConfig, newConfig)
		for i := 0; i < shardctrler.NShards; i++ {
			if lastConfig.Shards[i] == newConfig.Shards[i] {
				continue
			}
			if lastConfig.Shards[i] == kv.gid { // last assigned shard change
				kv.Shards[i].Status = ShardHandoff
				kv.rf.Start(ServerOp{
					Type: "Handoff",
					Num: lastConfig.Num,
					Sid: i,
				})
			} else if newConfig.Shards[i] == kv.gid { // get new assigned shard
				kv.Shards[i].Status = ShardHandoff
				kv.rf.Start(ServerOp{
					Type: "Migration",
					Num: lastConfig.Num,
					Servers: lastConfig.Groups[lastConfig.Shards[i]],
					Sid: i,
				})
			}
		}
		...
		kv.config = newConfig
		DPrintf("(%d:%d) current shard: %v",kv.gid, kv.me, kv.Shards)
		kv.mu.Unlock()
		time.Sleep(PollInterval * time.Millisecond)
	}
}
...
func (kv *ShardKV) ShardMigration(args *ShardMigrationArgs, reply *ShardMigrationReply) {
	...
	if args.Num > kv.config.Num || kv.Shards[args.Sid].Status != ShardMigratable {
		reply.Err = ErrShardPreparing
		return 
	}

	data := make(map[string]string)
	for k, v := range kv.Shards[args.Sid].Data {
		data[k] = v
	}
	reply.Data = data
}

func (kv *ShardKV) sendShardMigration(servers []string, sid int, num int) {
	kv.mu.Lock()
	if len(servers) == 0 {
	...
	args := ShardMigrationArgs{Num: num, Sid: sid}
