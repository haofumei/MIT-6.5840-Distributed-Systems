...
func TestReElection2A(t *testing.T) {
	...
	DPrintf("%d leader disconnects", leader1)
	...
	DPrintf("%d leader rejoin", leader1)
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()

	// if there's no quorum, no new leader should
	// be elected.
	DPrintf("%d and %d disconnect", leader2, (leader2 + 1) % servers)
	...
	DPrintf("%d rejoin", (leader2 + 1) % servers)
	cfg.connect((leader2 + 1) % servers)
	cfg.checkOneLeader()

	// re-join of last node shouldn't prevent leader from existing.
	DPrintf("%d rejoin", leader2)
	cfg.connect(leader2)
	cfg.checkOneLeader()
...
func TestManyElections2A(t *testing.T) {
	...
	for ii := 1; ii < iters; ii++ {
		...
		cfg.disconnect(i2)
		cfg.disconnect(i3)
		DPrintf("%d， %d, %d disconnect", i1, i2, i3)
...
func TestFailAgree2B(t *testing.T) {
	...
	DPrintf("%d disconnect", (leader + 1) % servers)
	// the leader and remaining follower should be
	// able to agree despite the disconnected follower.
	cfg.one(102, servers-1, false)
	cfg.one(103, servers-1, false)
	...
	DPrintf("%d connect", (leader + 1) % servers)
